...
public class MultiPoseDecoder {

    private static final String TAG = MultiPoseDecoder.class.getSimpleName();
...
//    private Segments segments;
    ...
    public MultiPoseDecoder(HeatmapScores heatmapScores, Offsets offsets, Displacements displacementsFwd, Displacements displacementsBwd) {
...
//        this.segments = segments;
    ...
    public List<Pose> decodeMultiplePoses(int outputStride, int maxPoseDetections, float scoreThreshold, float nmsRadius, int localMaxRadius) {
        ...
        while (poses.size() < maxPoseDetections && !scoreQueue.isEmpty()) {
            ...
            poses.add(new Pose(keypoints, poseScore));
        }
    ...
    private Keypoint[] decodePose(PartScore root, int outputStride) {
        int numParts = PoseTypes.PART_NAMES.length;
        int numEdges = PoseTypes.PARENT_TO_CHILD_EDGES.length;
        Keypoint[] instanceKeypoints = new Keypoint[numParts];
        PointF rootCoord = getImageCoordinates(root, outputStride);
        instanceKeypoints[root.getKeypointId()] = new Keypoint(root.getKeypointId(), rootCoord, root.getScore());

        // Decode the part positions upwards in the tree, following the backward
        // displacements.
        for (int edgeIndex = numEdges - 1; edgeIndex >= 0; edgeIndex--) {
            int sourceKeypointId = PoseTypes.PARENT_TO_CHILD_EDGES[edgeIndex];
            int targetKeypointId = PoseTypes.CHILD_TO_PARENT_EDGES[edgeIndex];
        ...
        for (int edgeIndex = 0; edgeIndex < numEdges; edgeIndex++) {
            int sourceKeypointId = PoseTypes.CHILD_TO_PARENT_EDGES[edgeIndex];
            int targetKeypointId = PoseTypes.PARENT_TO_CHILD_EDGES[edgeIndex];
    ...
    private Keypoint traverseToTargetKeypoint(Displacements displacements, int edgeIndex, Keypoint sourceKeypoint, int targetKeypointId, int outputStride) {
        ...
        return new Keypoint(targetKeypointId, targetKeypointPosition, score);
    }
    ...
    private PointF getImageCoordinates(Part part, int outputStride) {
        float dx = offsets.getOffsetX(part.getKeypointId(), part.getHeatMapScoresX(), part.getHeatMapScoresY());
        float dy = offsets.getOffsetY(part.getKeypointId(), part.getHeatMapScoresX(), part.getHeatMapScoresY());

        return new PointF(part.getHeatMapScoresX() * outputStride + dx, part.getHeatMapScoresY() * outputStride + dy);
    }
     ...
     */
    public static List<Pair<Keypoint, Keypoint>> getConnectedKeypoints(Keypoint[] keypoints, float minConfidence) {
